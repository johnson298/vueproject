<template>
    <div id="data-list-list-view" class="data-list-container">
        <vs-popup class="popup-custom-768" title="Thêm mới chăm sóc" :active.sync="addCustomer" >
            ...
            <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
                ...
                <div class="flex flex-wrap-reverse items-center">
                    ...
                    <div class="p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-between text-lg font-medium text-base text-primary border border-solid border-primary" @click="addCustomer = true">
                        ...
                        <span class="ml-2 text-base text-primary">Thêm chăm sóc</span>
                    </div>
                </div>
            </div>
...
<script>
import AddCustomerCare from './AddCustomerCare.vue';
...
export default {
  ...
  computed: {
    ...mapState('customer_care', ['users', 'pagination', 'searchTerm', 'order', 'views', 'needReload'])
  },
  methods: {
    deleteEmployee(user){
      this.$vs.dialog({
    ...
    updateViews(index, e){
      this.$store.dispatch('customer_care/updateViews', {
        index: index,
    ...
    getData(page = 1) {
      ...
      this.$http.get('users', {
        ...
        thisIns.$store.dispatch('customer_care/updateTable', {
    ...
    handleSearch(searching) {
      if (!this.needReload) {
        this.$store.dispatch('customer_care/updateNeedReload', true);
        return false;
      }
      let thisInt = this;
      this.$store.dispatch('customer_care/updateSearch', {
    ...
    handleSort(key, active) {
      this.$store.dispatch('customer_care/updateOrder', {
        order: {
          orderBy: key,
  ...
  destroyed() {
    this.$store.dispatch('customer_care/updateNeedReload', false);
  }
};
</script>
