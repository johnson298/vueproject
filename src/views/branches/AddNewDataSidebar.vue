<template>
    <vs-sidebar click-not-close position-right parent="body" default-index="1" color="primary" class="add-new-data-sidebar items-no-padding" spacer v-model="isSidebarActiveLocal">
            ...
            <div class="p-6">
                <form>
                    <div>
                        ...
                        <!--tên chi nhánh-->
                        <div>
                            <vs-input label="Tên chi nhánh" name="name" v-model="branches.name" class="mt-5 w-full" v-validate="'required'" />
                            <small class="text-danger">{{ errors.first('name') }}</small>
                        </div>
                        <div>
                            <vs-input label="Địa chỉ" name="address" v-model="branches.address" class="mt-5 w-full" v-validate="'required'" />
        ...
        <div class="flex flex-wrap items-center justify-center p-6" slot="footer">
            <vs-button class="mr-6 vs-con-loading__container" @click="createBranches" :disabled="errors.any()" ref="addButton" id="button-with-loading">Thêm</vs-button>
...
export default {
  ...
  methods: {
    ...
    createBranches() {
      ...
      this.$http.post('branches', this.formData(), { headers: { 'Content-Type': 'multipart/form-data' } })
        .then(() => {
          this.$vs.notify({
