...
<div>
    <vs-row vs-w="12">
        ...
        <vs-col vs-w="12">
            <h5><span class="text-primary">Thành tiền:</span> {{ DOCSO(expense.amount) }}</h5>
        </vs-col>
        ...
        <vs-col class="mt-5" vs-w='12' vs-type="flex" vs-justify="flex-end">
            <vs-button
              ...
              @click="createExpense"
              ref="addButton"
              id="btn-loading">Tạo chi</vs-button>
            <vs-button
              :disabled="(expense.amount&&expense.note) ? false : true"
              class="ml-3"
              type="filled"
              color="primary"
              @click="createExpense">Tạo & in hóa đơn</vs-button>
...
export default {
  ...
  methods: {
    ...
    createExpense() {
      ...
      this.$http.post('invoices?type=2', {
