...
<div id="data-list-list-view" class="data-list-container">
    <vs-popup class="popup-custom-768" title="Thêm mới hóa đơn" :active.sync="addExpen">
        <AddExpense :callback="getData" :active.sync="addExpen" @closePopupExpense="addExpen = $event" />
    </vs-popup>

    <vs-table-custom :sst="true" ref="table" multiple v-model="selected" @search="handleSearch" @sort="handleSort" :data="expense" search id="table" maxItems="10">
        ...
        <div slot="header" class="flex flex-wrap-reverse items-center flex-grow justify-between">
            ...
            <div class="flex flex-wrap-reverse items-center">
                ...
                <!-- ADD NEW -->
                <div class="p-3 mb-4 mr-4 rounded-lg cursor-pointer flex items-center justify-between text-lg font-medium text-base text-primary border border-solid border-primary" @click="addExpen=true">
                    <feather-icon icon="PlusIcon" svgClasses="h-4 w-4" />
                    <span class="ml-2 text-base text-primary">Thêm chi tiêu</span>
                </div>
            </div>
            ...
            <vs-tr :data="tr" :key="indextr" v-for="(tr, indextr) in data" class="col">
                ...
                <vs-td v-if="views.position.viewable">
                    <p class="product-name font-medium">
                        <vs-chip :color="checkStatus(positions,tr.user.position)=='Giáo viên' ? 'danger' 
...
export default {
  ...
  methods: {
    deleteExpense(expense) {
      this.$vs.dialog({
        ...
        accept: this.expenseAlert,
        parameters: [expense.id]
      });
    },
    expenseAlert(id) {
      this.$http.delete(`invoices/${id}?type=2`).then(() => {
        this.$vs.notify({
          color: 'success',
          title: 'Xóa hóa đơn',
    ...
    updateViews(index, e) {
      this.$store.dispatch('expense/updateViews', {
        index: index,
    ...
    getData(page = 1) {
      ...
      this.$http.get('invoices?type=2', {
        ...
        thisIns.$store.dispatch('expense/updateTable', {
          expense: thisIns.formatData(response.data.data),
    ...
    handleSearch(searching) {
      if (!this.needReload) {
        this.$store.dispatch('expense/updateNeedReload', true);
        return false;
      }
      let thisInt = this;
      this.$store.dispatch('expense/updateSearch', {
    ...
    handleSort(key, active) {
      this.$store.dispatch('expense/updateOrder', {
        order: {
          orderBy: key,
  ...
  destroyed() {
    this.$store.dispatch('expense/updateNeedReload', false);
  }
};
</script>
